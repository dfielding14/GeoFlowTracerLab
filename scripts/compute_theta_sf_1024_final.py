...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "campaigns" / "alpha_kappa_1024" / "theta_structure_functions"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compute theta structure functions (final snapshot) for 1024 campaign")
    ...
    p.add_argument("--fit-lo", type=float, default=DEFAULT_FIT_LO)
    p.add_argument("--fit-hi", type=float, default=None, help="Default: N/4 from each run's grid")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for i, run in enumerate(runs):
        ...
        fit_hi = float(args.fit_hi) if args.fit_hi is not None else float(run.grid) / 4.0
        ...
        try:
            ...
            plot_theta_sf(r, S, orders_arr, title, png_path, fit_lo=float(args.fit_lo), fit_hi=fit_hi)
